...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	while(p && monsterLoad)
	{
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"type"))
		{
			if(readXMLInteger(p, "first", intValue))
				mType->typingFirst = intValue;
			else
			{
				SHOW_XML_ERROR("Missing first type");
				monsterLoad = false;
			}

			if(readXMLInteger(p, "second", intValue))
				mType->typingSecond = intValue;
		}
