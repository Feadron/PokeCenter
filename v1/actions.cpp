...
ReturnValue Actions::canUse(const Player* player, const Position& pos, const Item* item)
{
	...
	if((action = getAction(item, ACTION_UNIQUEID)))
		return action->canExecuteAction(player, pos, false);
...
ReturnValue Actions::internalUseItemEx(Player* player, const PositionEx& fromPosEx, const PositionEx& toPosEx,
	Item* item, bool isHotkey, bool fishing, uint32_t creatureId)
{
    /*
    if(item->getUniqueOwner() != "" && !player->isItemUniqueOwner(item->getUniqueOwner()))
	    return RET_YOUARENOTTHEOWNER;
    */
	...
	if((action = getAction(item, ACTION_UNIQUEID)))
	{
		ReturnValue ret = action->canExecuteAction(player, toPosEx, false);
		if(ret != RET_NOERROR)
			return ret;
	...
	if((action = getAction(item, ACTION_ACTIONID)))
	{
	 ...
	 if((action = getAction(item, ACTION_ITEMID)))
	{
		ReturnValue ret = action->canExecuteAction(player, toPosEx, fishing);
		if(ret != RET_NOERROR)
			return ret;
	...
	uint8_t toStackPos, Item* item, bool isHotkey, uint32_t creatureId/* = 0*/)
{
	...
	ReturnValue ret = internalUseItemEx(player, fromPosEx, toPosEx, item, isHotkey, fishing, creatureId);
	if(ret != RET_NOERROR)
	{
		player->sendCancelMessage(ret);
		return false;
...
ReturnValue Action::canExecuteAction(const Player* player, const Position& toPos, bool fishing)
{
	if (fishing){
		return RET_NOERROR;
	}
