...
bool LuaScriptInterface::loadFile(const std::string& file, Npc* npc/* = NULL*/)
{
	...
	std::cout << "File path we want to know: " << file << std::endl;
...
void LuaScriptInterface::registerFunctions()
{
	...
	//doPlayerSendMarketMailByName(name, item[, town[, actor]])
	lua_register(m_luaState, "doPlayerSendMarketMailByName", LuaScriptInterface::luaDoPlayerSendMarketMailByName);
...
{
	//doPlayerSendPokemonWindowAddPokemonIcon(cid, itemId, fastcallNumber, textColor, text, pokeLevel)
	if(lua_gettop(L) < 8)
	{
		errorEx("doPlayerSendPokemonWindowAddPokemonIcon -> Less than 8 parameters!");
		lua_pushboolean(L, false);
		return 1;
    }
    
    uint8_t pokeExp = popNumber(L);
    uint16_t pokeGender = popNumber(L);
    uint16_t pokeMana = popNumber(L);
    uint16_t pokeMaxMana = popNumber(L);
    uint16_t pokeLevel = popNumber(L);
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		player->sendPokemonWindowAddPokemonIcon(itemId, fastcallNumber, textColor, text, pokeLevel, pokeMaxMana, pokeMana, pokeGender, pokeExp);
		lua_pushboolean(L, true);
	}
	else
	{
...
{
	//doPlayerSendPokemonWindowAddPokemonIcon(cid, fastcallNumber, textColor, text, pokeLevel, pokeMaxMana, pokeMana, pokeGender)
	if(lua_gettop(L) < 8)
	{
		errorEx("doPlayerSendPokemonWindowUpdatePokemonIcon -> Less than 8 parameters!");
		lua_pushboolean(L, false);
		return 1;
    }
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		player->sendPokemonWindowUpdatePokemonIcon(fastcallNumber, textColor, text, pokeLevel, pokeMaxMana, pokeMana, pokeGender, pokeExp);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaDoPlayerSendMarketMailByName(lua_State* L)
{
	//doPlayerSendMarketMailByName(name, item[, town[, actor]])
	ScriptEnviroment* env = getEnv();
	int32_t params = lua_gettop(L);
	...
	lua_pushboolean(L, IOLoginData::getInstance()->playerMarketMail(actor, popString(L), town, item));
	return 1;
}
