	...
	int32_t subType/* = -1*/, bool addArticle/* = true*/, Player* player/* = null*/)
{
    ...
    if (it.sellPrice > 0) {
        if (tmpItem) {
            s << std::endl << (player ? Localization::t(player->getLanguage(), "Price: $") : "Price: $") << (it.sellPrice * tmpItem->getItemCount());
            ...
            s << std::endl << (player ? Localization::t(player->getLanguage(), "Price: $") : "Price: $") << it.sellPrice << ".";;
        }
    }
