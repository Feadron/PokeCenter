...
uint64_t IOLoginData::createAccount(std::string name, std::string password, std::string email)
{
	...
	DBQuery query;
	query << "INSERT INTO `accounts` (`id`, `name`, `password`, `email`) VALUES (NULL, " << db->escapeString(name) << ", " << db->escapeString(password) << ", " << db->escapeString(email) << ")";
...
bool IOLoginData::playerMarketMail(Creature* actor, std::string name, uint32_t townId, Item* item)
{
...
bool IOLoginData::createCharacter(uint32_t accountId, std::string characterName, int32_t vocationId, uint16_t sex, uint8_t townId, uint8_t worldId)
{
